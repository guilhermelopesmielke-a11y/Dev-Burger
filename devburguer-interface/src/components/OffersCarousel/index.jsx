...
import { Container, Feedback, Title } from "./styles";
...
export function OffersCarousel() {
    ...
    return (
        <Container>
            ...
            {loading && <Feedback>Carregando ofertas...</Feedback>}

            {!loading && error && <Feedback $error>{error}</Feedback>}

            {!loading && !error && offers.length === 0 && (
                <Feedback>Nenhuma oferta cadastrada ainda.</Feedback>
            )}
            ...
            {!loading && !error && offers.length > 0 && (
                <Carousel
                    ...
                    arrows={false}
                    customButtonGroup={<CarouselArrows totalItems={offers.length} />}
                >
                    {offers.map((offer) => (
